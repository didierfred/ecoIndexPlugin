...
**/ 
function mesureSize(request) {

console.log("size=" request.response.bodySize);
byte_total = byte_total + request.response.bodySize; 
/**
...
function notify(message) 
	...
	if (json_message.status=="off")
		{
		removeListener();
		chrome.browserAction.setIcon({ path: "icons/ecoindex-32.png"});
	...
	if (json_message.status=="on")
		{
		addListener();
		chrome.browserAction.setIcon({ path: "icons/ecoindex-green-32.png"});
		nb_request=0;
		byte_total = 0;
...
function addListener()
	...
	chrome.webRequest.onBeforeRequest.addListener(countRequest,
                                          {urls: [target]});
...
//
		chrome.devtools.network.onRequestFinished.addListener(mesureSize
							);
	}
...
function removeListener()
	{
	chrome.webRequest.onBeforeRequest.removeListener(countRequest);
	chrome.webRequest.onBeforeRequest.removeListener(mesureSize);
	}
